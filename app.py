...
def create_app():
    ...
    def learn():
        ...
        glossary = [
            {"icon": "🧠", "title": "أساسيات للمتعلّم", "terms": [
                {"name": "الاتجاهات الثلاثة",
                 "desc": "اتجاه السهم له ثلاث حالات: صاعد (كل قمة وقاع أعلى من الذي قبله) · هابط (كل قمة وقاع أدنى من سابقه) · عرضي (يتحرّك أفقياً بلا اتجاه واضح). الأصل في التربّح هو الشراء في الاتجاه الصاعد. فهم هذه الحالات يساعدك على قراءة شارات الاتجاه في المنصة (EMA · ADX · سوبرترند).",
                 "example": None},
                {"name": "لماذا لا تتّفق كل المؤشرات؟",
                 "desc": "كل مؤشر يقيس زاوية مختلفة من السهم (الاتجاه · الزخم · الحجم · السيولة). فمن الطبيعي ألا تتّفق كلها — بل اختلافها صحّي: لو أعطت جميعها الإشارة نفسها دائماً لصارت مؤشراً واحداً مكرّراً. لذلك «قوة التأكيد» و«ميزان الإشارات» نادراً ما يبلغان 100%. المهم تضافر عدة أدلة، لا اتفاقها كلها.",
                 "example": None},
                {"name": "القيادي مقابل الخشاش",
                 "desc": "الشركة القيادية كبيرة القيمة السوقية، تقود سوقها، وحركتها أهدأ وأكثر ثقة — مثل الشركات الـ32 التي تتابعها المنصة. أما شركة «الخشاش» فصغيرة وضعيفة تتذبذب بعنف صعوداً وهبوطاً. تركّز المنصة على القياديات لأنها أوثق بياناتٍ وأقل تلاعباً وأنسب للمتعلّم المبتدئ.",
                 "example": None},
            ]},
            ...
            {"icon": "🕯️", "title": "شمعة الانعكاس", "terms": [
                {"name": "شمعة الانعكاس (Price Action)",
                 "desc": "شكل حركة السعر قد يمهّد لتغيّر الاتجاه: الابتلاع الصاعد/الهابط (شمعة تبتلع جسم ما قبلها) · المطرقة (ذيل سفلي طويل بعد هبوط) · الشهاب (ذيل علوي طويل بعد صعود) · الدوجي (فتح ≈ إغلاق = تردّد) · نجمة الصباح/المساء (ثلاث شموع: اتجاه ⇒ تردّد ⇒ حركة قوية عكسية — من أقوى الأنماط) · خط المخترق/الغيمة القاتمة (شمعة تُغلق فوق/تحت منتصف جسم ما قبلها). تنبيه معرفي فقط — شمعة واحدة دليل ضعيف حتى تؤكّدها الجلسات بعدها.",
                 "example": "ابتلاع صاعد بعد هبوط قد يعني بداية انعكاس للأعلى — لكن ننتظر تأكيد الأيام التالية."},
            ]},
            {"icon": "🧭", "title": "هيكل السوق (Smart Money)", "terms": [
                {"name": "تتابع القمم والقيعان (HH · HL · LH · LL)",
                 "desc": "نقرأ اتجاه السهم من تتابع قممه وقيعانه: HH قمة أعلى · HL قاع أعلى (تتابعهما = اتجاه صاعد) · LH قمة أدنى · LL قاع أدنى (تتابعهما = اتجاه هابط).",
                 "example": "HH ثم HL ثم HH = هيكل صاعد صحّي."},
                {"name": "BOS — كسر الهيكل (استمرار)",
                 "desc": "تجاوز السعر آخر قمة/قاع مهم في نفس اتجاهه = تأكيد استمرار الاتجاه. صعوداً نسمّيه «اختراق»، وهبوطاً «كسر».",
                 "example": None},
                {"name": "CHOCH — تغيّر الطابع (بداية انعكاس)",
                 "desc": "أول تجاوز عكس الاتجاه السائد = إنذار مبكّر باحتمال انقلاب الاتجاه. أهم إشارة، لذلك نضعها أولاً في صفحة هيكل السوق.",
                 "example": None},
                {"name": "إعادة الاختبار (Retest)",
                 "desc": "بعد تجاوز مستوى، يعود السعر أحياناً ليختبره: إن ثبت فوقه (بعد اختراق) أو تحته (بعد كسر) صار «مؤكّداً» — أقوى نقطة في المفهوم (المقاومة تتحوّل دعماً والعكس).",
                 "example": None},
                {"name": "الاختراق مقابل الكسر",
            ...
            {"icon": "⏱️", "title": "الفريمات الثلاثة", "terms": [
                {"name": "تأكيد الفريمات (يومي · أسبوعي · شهري)",
                 "desc": "حالة السهم على ثلاثة أُطر زمنية معاً، محسوبة من أسعاره نفسها (بلا تكلفة). اتجاه كل فريم يُقرأ بتتابع القمم والقيعان (نفس منطق هيكل السوق) لأنها الأساس في تحديد الاتجاه، بينما تبقى المتوسطات والمؤشرات من معايير التحليل الأخرى المعتمدة في المنصة. الفكرة أن الاتجاه الأقوى هو ما اتّفقت عليه الفريمات: 🟢 إيجابي (صاعد) · 🔴 سلبي (هابط) · ⚪ محايد.",
                 "example": "سهم يومي+أسبوعي+شهري صاعد = اتجاه متين على كل المستويات."},
                {"name": "قوة الفريمات (متناظرة: صعوداً وهبوطاً)",
                 "desc": "تقيس توافق الفريمات في الاتجاهين: 🟢 ثلاثية صاعدة = الثلاثة صاعدة (أقوى شراءً) · 🟢 ثنائية صاعدة = فريمان صاعدان · ⚪ عرضية = لا توافق واضح · 🔴 ثنائية هابطة = فريمان هابطان · 🔴 ثلاثية هابطة = الثلاثة هابطة (أقوى هبوطاً — ابتعاد). القاعدة الذهبية: لا تدرس الدخول إلا والفريم الأكبر إيجابي.",
                 "example": "سهم قوّته ثلاثية أجدر بالدراسة من سهم إيجابي يومياً فقط."},
                {"name": "كيف يُقاس اتجاه كل فريم؟",
                 "desc": "بالقمم والقيعان المتتابعة (HH/HL صعوداً · LH/LL هبوطاً) — وهي الأساس في قراءة الاتجاه (داو/سمارت موني). أما المتوسطات والمؤشرات فتبقى معايير تحليل معتبَرة تُكمّل الصورة في مواضع أخرى بالمنصة. الأسبوعي والشهري نبنيهما بتجميع أسعار خمس سنوات كاملة، فتكفي شموعهما للقمم والقيعان. وصفي تعليمي — لا توصية.",
                 "example": None},
            ]},
            ...
            {"icon": "🕵️", "title": "المطّلعون (Insiders)", "terms": [
                {"name": "معاملات المطّلعين (EDGAR)",
                 ...
                 "example": None},
                {"name": "الشراء العنقودي",
                 "desc": "أن يشتري مطلعان مختلفان أو أكثر أسهم شركتهم من السوق المفتوح بأموالهم الخاصة — قناعة جماعية من عدة أشخاص يعرفون الشركة من الداخل، أقوى من رأي فرد واحد. العبرة بعدد الأشخاص المختلفين، لا عدد الصفقات.",
                 "example": "شخصان مختلفان يشتريان نفس السهم = شراء عنقودي. أما شخص واحد اشترى 7 مرات فليس عنقودياً."},
            ]},
            ...
            {"icon": "👑", "title": "مؤشر Algomatix", "terms": [
                {"name": "الدرجة الموزونة (0–100)", "id": "algomatix",
                 "desc": "مبني على 9 مدارس تحليلية مجموع أوزانها 100 (هيكل السوق · الجودة والنمو · الاتجاه · الزخم · السيولة · المستويات · الفريمات الثلاثة · القوة النسبية · الشموع)، تجمع 16 معياراً فنياً ومالياً + تأكيد الفريمات الثلاثة في درجة موحّدة (0–100) تلخّص قوّة كل سهم برقم واحد. 🟢 قوية (70+) 🟡 جيدة (55–69) ⚪ محايد (40–54) 🔴 ضعيف. هو المقياس الأشمل لأنه يضيف الأساسيات المالية وتوافق الفريمات ويوزن كل مدرسة بأهميتها — بخلاف «مجموع نقاط الاستراتيجيات» الفني البحت.",
                 "example": "درجة 81 = تضافر قوي عبر المدارس التسع معاً."},
            ]},
            ...
            {"icon": "🧭", "title": "خطة التداول (ATR)", "terms": [
                ...
                {"name": "الدخول / الوقف / الهدف",
                 ...
                 "example": None},
                {"name": "قرب المقاومة (قاعدة الانتظار)",
                 "desc": "تنبيه يظهر في صفحة السهم حين يكون السعر ملاصقاً لمقاومة سابقة لم تُخترق بعد. الشراء عند مستوى قمة سابقة يكثر عنده الارتداد — فالأفضل انتظار تأكيد الاختراق أولاً. وصفي تعليمي لا توصية.",
                 "example": None},
            ]},
            {"icon": "📋", "title": "نموذج خطط التداول", "terms": [
                {"name": "الاستراتيجيات الـ13 ودرجة الانطباق",
                 "desc": "نطبّق 13 استراتيجية فنية على كل سهم، وكل واحدة تأخذ «درجة انطباق من 10» حسب قوّة تحقّقها (اتجاه · تقاطع ذهبي · MACD · RSI · ADX · سوبرترند · اختراق · بولينجر · حجم · OBV · سيولة · أقوى من السوق · شمعة انعكاس).",
                 "example": None},
                {"name": "الاستراتيجيات المتحققة",
                 "desc": "عدد الاستراتيجيات التي بلغت درجتها 7 فأكثر (منطبقة بقوة) من أصل 13. كلما زادت، زاد تضافر الأدلة الفنية.",
                 "example": "6 من 13 = ست استراتيجيات قوية متحققة."},
                {"name": "مجموع نقاط الاستراتيجيات الـ13",
                 "desc": "مجموع درجات الـ13 استراتيجية مجتمعة (كل واحدة 0–10). مقياس فني بحت للقوة الإجمالية — لا يشمل الأساسيات المالية (بخلاف مؤشر Algomatix).",
                 "example": None},
                {"name": "الخلاصة: مكتمل / انتظار / ضعيف",
                 "desc": "قاعدة الدخول التعليمية: «مكتمل الشروط للدراسة» تحتاج ≥3 استراتيجيات منطبقة + صفقة رابحة (العائد يفوق المخاطرة) + بُعد عن المقاومة. إن نقص شرط ⇒ «انتظار»، وإن غابت الشروط ⇒ «ضعيف». وصفي تعليمي لا توصية.",
                 "example": None},
            ]},
            {"icon": "⚡", "title": "قوة الإشارات", "terms": [
                {"name": "دقة النوع تاريخياً",
                 "desc": "نسبة نجاح هذا النوع من الإشارات عبر تاريخه، مع «عدد الإشارات» (حجم العيّنة). كلما كبر العدد زادت مصداقية النسبة.",
                 "example": "61% (23 إشارة) أوثق بكثير من 100% (إشارة واحدة)."},
                {"name": "متوسط العائد",
                 "desc": "متوسط عائد الإشارة الواحدة من هذا النوع — المقياس الأصدق للربحية لأنه يجمع «كم مرة أصاب» و«كم ربح» معاً في رقم واحد.",
                 "example": None},
                {"name": "القوة (قوي / سلبي / قليل التكرار)",
                 "desc": "وسم يلخّص جدارة النوع في صفحة الإشارات: 🥇 قوي (عيّنة كافية ≥10 + متوسط عائد موجب) · 🔻 سلبي (عيّنة كافية + عائد سالب مثبت) · ⚠️ قليل التكرار (ظهر أقل من 10 مرات، فسجلّه غير كافٍ للحكم عليه بعد).",
                 "example": None},
            ]},
            {"icon": "📊", "title": "اختبار الأداء", "terms": [
                {"name": "نسبة النجاح",
                 "desc": "كم إشارة ربحت من الإجمالي (الصاعدة: صعد السهم؛ الهابطة: نزل). تقيس «كم مرة أصابت» — لا حجم الربح (تُقرأ مع متوسط العائد).",
                 "example": None},
                {"name": "مقابل السوق (الألفا)",
                 "desc": "هل تفوّقت الإشارة على مؤشر السوق (S&P 500) عن نفس الفترة؟ موجب = أداؤها أفضل من مجرّد شراء السوق.",
                 "example": None},
                {"name": "النضج (لم تنضج)",
                 "desc": "الإشارة الطازجة (أقل من 3 أيام) لم يمرّ عليها وقت كافٍ للحكم، فتُعرض في الجدول لكن لا تدخل الإحصائيات، وتُوضع أسفل جدول اختبار الأداء (لأنها لم تُختبر بعد).",
                 "example": None},
            ]},
        ]
